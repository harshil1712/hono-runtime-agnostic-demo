...
const isContainerExitNonZeroError = (error) => isErrorOfType(error, UNEXPECTED_EDIT_ERROR);
function getExitCodeFromError(error) {
    ...
    if (isContainerExitNonZeroError(error)) {
        return +error.message
            .toLowerCase()
            .slice(error.message.toLowerCase().indexOf(UNEXPECTED_EDIT_ERROR) +
...
export class Container extends DurableObject {
    ...
    async fetch(request) {
        if (this.defaultPort === undefined) {
            return new Response('No default port configured for this container. Override the fetch method or set defaultPort in your Container subclass.', { status: 500 });
        }
